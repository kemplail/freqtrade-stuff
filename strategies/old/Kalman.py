...
class Kalman(IStrategy):
    ...
    cstop_bail_how = "time"
    cstop_bail_roc = -1.425
    cstop_bail_time = 832
    cstop_bail_time_trend = True
    cstop_loss_threshold = -0.033
    cstop_max_stoploss = -0.093
    exit_kf_diff = -4.0
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative['kf_predict'] = informative['close'].rolling(
            window=self.kf_window).apply(self.model)
        ...
        dataframe['kf_predict_diff'] = 100.0 * \
            (dataframe['kf_predict'] - dataframe['close']) / dataframe['close']
    ...
    def model(self, a: np.ndarray) -> np.float:
        ...
        if not self.filter_init_list[self.current_pair]:
            ...
            self.filter_list[self.current_pair] = self.filter_list[self.current_pair].em(
                scaled, n_iter=6)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Kalman triggers
        kf_cond = (
                qtpylib.crossed_above(dataframe['kf_predict_diff'], self.entry_kf_diff)
        )
        ...
        spike_cond = (
                dataframe['kf_predict_diff'] < 2.0 * self.entry_kf_diff
        )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Kalman triggers
        kf_cond = (
                qtpylib.crossed_below(dataframe['kf_predict_diff'], self.exit_kf_diff)
        )
        ...
        spike_cond = (
                dataframe['kf_predict_diff'] > 2.0 * self.exit_kf_diff
        )
        conditions.append(spike_cond)
