...
class DWT_Leveraged_recentHO(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if (self.isBull(curr_pair)) or (self.isBear(curr_pair)):
            ...
            inf_slow['dwt_model'] = inf_slow['close'].rolling(
                window=self.dwt_window).apply(self.model)

            # trend (in informative)
            inf_fast['candle-up'] = np.where(inf_fast['close'] >= inf_fast['open'], 1, 0)
            inf_fast['candle-up-trend'] = np.where(
                inf_fast['candle-up'].rolling(5).sum() >= 3, 1, 0)
            inf_fast['candle-dn-trend'] = np.where(
                inf_fast['candle-up'].rolling(5).sum() <= 2, 1, 0)

            # merge into normal timeframe
            dataframe = merge_informative_pair(
                dataframe, inf_slow, self.timeframe, self.inf_timeframe, ffill=True)
            dataframe = merge_informative_pair(
                dataframe, inf_fast, self.timeframe, self.timeframe, ffill=True)
            ...
            dataframe['dwt_model_diff'] = 100.0 * \
                (dataframe['dwt_model'] - dataframe['inf_close']) / dataframe['inf_close']
            ...
            dataframe['candle-dn-trend'] = np.where(
                dataframe['candle-up'].rolling(5).sum() <= 2, 1, 0)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 'Bear'/short leveraged token
            ...
            # DWT triggers
            short_dwt_cond = (
                    qtpylib.crossed_below(dataframe['dwt_model_diff'],
                                          self.entry_short_dwt_diff)
            )
